"""
file_processor.py — AURA Universal File Processor
...
# Modern google.genai imported dynamically in _generate_text
...
def _generate_text(prompt: str) -> str:
    ...
    try:
        ...
        try:
            ...
            return f"AI processing failed: {e} | {legacy_e}"
...
def _gemini_client():
    try:
        ...
        return legacy_genai.GenerativeModel("gemini-2.0-flash")
    except Exception:
        return None
...
def _process_pdf(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    inst = params.get("instruction", "")
    prompt = f"Analyze this PDF document concisely:\n\n{text}"
    if inst:
...
def _process_text_doc(path: Path, file_type: str, action: str,
    ...
    try:
        result   = _generate_text(prompt_map[action])
        if len(result) > 600 and params.get("save", True):
