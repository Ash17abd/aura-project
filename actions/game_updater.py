...
def _schedule_daily_update(hour: int = 3, minute: int = 0) -> str:
    ...
    subprocess.run(["schtasks", "/Delete", "/TN", "AURE_GameUpdater", "/F"], capture_output=True)
...
def _cancel_scheduled_update() -> str:
    subprocess.run(["schtasks", "/Delete", "/TN", "AURE_GameUpdater", "/F"], capture_output=True)
    result = subprocess.run(["schtasks", "/Delete", "/TN", "AURA_GameUpdater", "/F"],
                            capture_output=True, text=True)
    return "Scheduled update cancelled." if result.returncode == 0 else "No scheduled update found."


def _get_schedule_status() -> str:
    result = subprocess.run(["schtasks", "/Query", "/TN", "AURA_GameUpdater", "/FO", "LIST"],
                            capture_output=True, text=True)
    if result.returncode != 0:
        result = subprocess.run(["schtasks", "/Query", "/TN", "AURE_GameUpdater", "/FO", "LIST"],
                                capture_output=True, text=True)
    if result.returncode != 0:
