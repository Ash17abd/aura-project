...
PROJECTS_DIR     = Path.home() / "Desktop" / "AuraProjects"
MAX_FIX_ATTEMPTS = 5
...
class _DevGeminiWrapper:
    ...
    def generate_content(self, contents, **kwargs):
        try:
            ...
            resp = client.models.generate_content(model=self.model_name, contents=contents)
            class _Resp:
                def __init__(self, text):
                    self.text = text
            return _Resp(resp.text or "")
        except Exception:
...
def _get_model(model_name: str):
    return _DevGeminiWrapper(model_name)
...
def _build_project(
    ...
    proj_name    = project_name or plan.get("project_name", "aura_project")
