...
PLANNER_PROMPT = """You are the planning module of AURA, a personal AI assistant.
...
open_3d_lab
  user_instruction: string (optional) — prompt to generate or search 3D model (e.g. "transformer", "rocket engine", "electric motor")
  file_path: string (optional) — reading file path
...
def _generate_planner_text(prompt: str) -> str:
...
def create_plan(goal: str, context: str = "") -> dict:
    ...
    try:
        text = _generate_planner_text(user_input)
...
Generate a revised plan for the remaining work only. Do not repeat completed steps."""

    try:
        text = _generate_planner_text(prompt)
        text = re.sub(r"```(?:json)?", "", text).strip().rstrip("`").strip()
        plan = json.loads(text)
        print(f"[Planner] [REPLAN] Revised plan: {len(plan.get('steps', []))} steps")
        return plan
    except Exception as e:
